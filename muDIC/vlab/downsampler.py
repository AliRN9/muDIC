...
def chess_board(n):
    ...
    tile_assembly = np.tile(tile, (n, n)).astype(np.float64)
...
class Downsampler(object):
    ...
    def __generate_coordinates__(self):
        ...
        n_subpoints = np.floor(self.factor * np.sqrt(self.fill)).astype(np.int32)

        num_segments_n = np.float64(self.dim_n) / self.factor
        num_segments_m = np.float64(self.dim_m) / self.factor

        tile_i = np.arange(num_segments_n, dtype=np.float64)
        tile_j = np.arange(num_segments_m, dtype=np.float64)
        ...
        ind_i, ind_j = np.meshgrid(np.arange(n_subpoints, dtype=np.float64), np.arange(n_subpoints, dtype=np.float64))

        subframe_i = (coord_subpos(i, self.fill, n_subpoints, ind_i,
                                   self.pixel_offset_stddev) + .5) * np.float64(
            self.factor) - .5
        ...
        coordinates = np.zeros(
            (2, np.int32(num_segments_n), np.int32(num_segments_m), n_subpoints, n_subpoints))
